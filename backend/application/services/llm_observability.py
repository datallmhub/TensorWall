...
class LLMObservabilityService:
    ...
    async def get_governance_kpis(
        ...
        async with get_db_context() as db:
            ...
            cost_breakdown = await self._calculate_cost_breakdown_from_traces(
                db, traces
            )
            ...
            anomalies = await self._detect_anomalies_from_traces(
                db, traces, start_time, end_time
            )
            ...
            return GovernanceKPIs(
                ...
                cost_per_1k_tokens=(
                    round((total_cost / total_tokens) * 1000, 4)
                    if total_tokens > 0
                    else 0
                ),
            )
    ...
    def _calculate_blocking_stats_from_traces(
        self, traces: List[LLMRequestTrace]
    ) -> BlockingStats:
        """
        ...
        rate_limit_blocks = sum(
            1 for t in traces if "rate_limit" in (t.decision_reasons or [])
        )
        ...
        security_blocks = sum(
            ...
            for t in traces
            if any(
                cat in ["prompt_injection", "pii_leakage"]
                for cat in (t.risk_categories or [])
            )
        )

        # Calculate cost avoided
        cost_avoided = sum(
            t.estimated_cost_avoided
            for t in traces
        ...
        top_blocked = [
            ...
            for app_id, count in sorted(
                blocked_by_app.items(), key=lambda x: x[1], reverse=True
            )[:10]
        ]
    ...
    async def _calculate_cost_breakdown(
        ...
        by_app_items = [
            CostBreakdownItem(
                ...
                percentage=(
                    round((stats["cost_usd"] / total_cost) * 100, 2)
                    if total_cost > 0
                    else 0
                ),
            )
        ...
        by_env_items = [
            ...
            for env, stats in sorted(
                by_env.items(), key=lambda x: x[1]["cost_usd"], reverse=True
            )
        ]
    ...
    def _calculate_token_efficiency(
        ...
        return TokenEfficiency(
            input_output_ratio=(
                round(total_output / total_input, 2) if total_input > 0 else 0
            ),
            avg_tokens_per_request=round(
                (total_input + total_output) / total_requests, 1
            ),
    ...
    async def _detect_anomalies(
        ...
        if baseline_cost > 0:
            cost_increase_percent = (
                (current_cost - baseline_cost) / baseline_cost
            ) * 100

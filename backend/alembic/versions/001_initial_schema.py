...
def upgrade() -> None:
    # Create enum types
    environment_enum = postgresql.ENUM('development', 'staging', 'production', name='environment', create_type=False)
    environment_enum.create(op.get_bind(), checkfirst=True)

    budget_period_enum = postgresql.ENUM('hourly', 'daily', 'weekly', 'monthly', name='budgetperiod', create_type=False)
    budget_period_enum.create(op.get_bind(), checkfirst=True)

    budget_scope_enum = postgresql.ENUM('APPLICATION', 'USER', 'FEATURE', 'ORGANIZATION', name='budgetscope', create_type=False)
    budget_scope_enum.create(op.get_bind(), checkfirst=True)

    policy_action_enum = postgresql.ENUM('allow', 'warn', 'deny', name='policyaction', create_type=False)
    policy_action_enum.create(op.get_bind(), checkfirst=True)

    rule_type_enum = postgresql.ENUM('model_restriction', 'environment_restriction', 'feature_restriction', 'token_limit', 'time_restriction', 'general', name='ruletype', create_type=False)
    rule_type_enum.create(op.get_bind(), checkfirst=True)

    audit_event_enum = postgresql.ENUM('request', 'response', 'policy_decision', 'budget_check', 'security_check', 'error', name='auditeventtype', create_type=False)
    audit_event_enum.create(op.get_bind(), checkfirst=True)

    trace_decision_enum = postgresql.ENUM('allow', 'warn', 'block', 'degrade', name='tracedecision', create_type=False)
    trace_decision_enum.create(op.get_bind(), checkfirst=True)

    trace_status_enum = postgresql.ENUM('pending', 'success', 'blocked', 'error', 'timeout', name='tracestatus', create_type=False)
    trace_status_enum.create(op.get_bind(), checkfirst=True)

    tenant_status_enum = postgresql.ENUM('ACTIVE', 'SUSPENDED', 'DELETED', name='tenantstatus', create_type=False)
    tenant_status_enum.create(op.get_bind(), checkfirst=True)

    tenant_tier_enum = postgresql.ENUM('FREE', 'STARTER', 'PROFESSIONAL', 'ENTERPRISE', name='tenanttier', create_type=False)
    ...
    op.create_table('organizations',
        ...
        sa.Column('status', sa.Enum('ACTIVE', 'SUSPENDED', 'DELETED', name='tenantstatus'), nullable=False, server_default='ACTIVE'),
        sa.Column('tier', sa.Enum('FREE', 'STARTER', 'PROFESSIONAL', 'ENTERPRISE', name='tenanttier'), nullable=False, server_default='FREE'),
        ...
        sa.Column('audit_retention_days', sa.Integer(), nullable=False, server_default='30'),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=False),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table('api_keys',
        ...
        sa.Column('environment', sa.Enum('development', 'staging', 'production', name='environment'), nullable=False, server_default='development'),
    ...
    op.create_table('policy_rules',
        ...
        sa.Column('name', sa.String(length=255), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('rule_type', sa.Enum('model_restriction', 'environment_restriction', 'feature_restriction', 'token_limit', 'time_restriction', 'general', name='ruletype'), nullable=False),
        sa.Column('action', sa.Enum('allow', 'warn', 'deny', name='policyaction'), nullable=False),
    ...
    op.create_index('ix_policy_rules_user_email', 'policy_rules', ['user_email'], unique=False)

    # Budgets table
    op.create_table('budgets',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('uuid', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('scope', sa.Enum('APPLICATION', 'USER', 'FEATURE', 'ORGANIZATION', name='budgetscope'), nullable=False),
        ...
        sa.Column('period', sa.Enum('hourly', 'daily', 'weekly', 'monthly', name='budgetperiod'), nullable=False, server_default='monthly'),
    ...
    op.create_table('audit_logs',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('uuid', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('event_type', sa.Enum('request', 'response', 'policy_decision', 'budget_check', 'security_check', 'error', name='auditeventtype'), nullable=False),
        sa.Column('app_id', sa.String(length=100), nullable=True),
        sa.Column('user_email', sa.String(length=255), nullable=True),
    ...
    op.create_index('ix_usage_records_app_id', 'usage_records', ['app_id'], unique=False)
    ...
    op.create_table('request_traces',
        ...
        sa.Column('feature', sa.String(length=100), nullable=True),
        sa.Column('environment', sa.String(length=50), nullable=True),
        sa.Column('decision', sa.Enum('allow', 'warn', 'block', 'degrade', name='tracedecision'), nullable=False),
        sa.Column('status', sa.Enum('pending', 'success', 'blocked', 'error', 'timeout', name='tracestatus'), nullable=False),
    ...
    op.create_index('ix_request_traces_app_id', 'request_traces', ['app_id'], unique=False)
    op.create_index('ix_request_traces_created_at', 'request_traces', ['created_at'], unique=False)
    op.create_index('ix_request_traces_decision', 'request_traces', ['decision'], unique=False)
    op.create_index('ix_request_traces_model', 'request_traces', ['model'], unique=False)
    op.create_index('ix_request_traces_status', 'request_traces', ['status'], unique=False)
    ...
    op.create_index('ix_features_app_feature', 'features', ['application_id', 'name'], unique=True)
